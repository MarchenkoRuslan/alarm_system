...
| New market payload | `new_market` event includes `tags`, `condition_id`, `clob_token_ids`, and other metadata | https://docs.polymarket.com/developers/CLOB/websocket/market-channel | Scenario C arming priority: WS `new_market` first; fallback to Gamma discovery sync when WS custom feature stream is unavailable |
...
| Gamma tags filtering | `GET /events` and `GET /markets` support tag-based filtering via `tag_id` and related tag controls | https://docs.polymarket.com/developers/gamma-markets-api/get-markets | Scenario B and C category scoping should use tag-based filtering instead of free-text matching |
